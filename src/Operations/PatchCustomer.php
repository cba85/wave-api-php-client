...
class PatchCustomer extends Operation implements OperationInterface
{
    /**
     * Default fields
     *
    ...
    public function run(array $fields = [], array $arguments = [], array $variables = []): object
    {
        ...
        $customerPatchFields = implode(',', $queryFields['customerPatch']);
        $customerFields = implode(',', $queryFields['customer']);
        $addressFields = implode(',', $queryFields['address']);

        $mutation = (new Mutation('customerPatch'))
            ->setVariables([new Variable('input', 'CustomerPatchInput', true)])
        ...
        return $results->getData()->customerPatch;
    }
}
